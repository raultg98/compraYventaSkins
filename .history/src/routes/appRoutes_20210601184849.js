...
router.get('/skins/delete/:id', async (req, res, next) => {
    const { id } = req.params;
    const skins = await pool.query('DELETE from skins WHERE id_skin = ?', id);
...
router.get('/skins/edit/:id', async (req, res, next) => {
    ...
    // PASOC AL CLIENTE UN OBJETO QUE CONTIENE TODOS LOS DATOS DE LA SKIN
    res.render('admin/edit', { datos: datosSkin[0] });
});

router.post('/skins/edit/:id', async (req, res, next) => {
    const { id } = req.params;
    const { nombre, precio, foto, categoria, descripcion } = req.body;

    await pool.query('UPDATE skins SET nombre='+ nombre + ' precio='+ precio + ' where id_skin='+ id);
    res.redirect('/skins');
});
